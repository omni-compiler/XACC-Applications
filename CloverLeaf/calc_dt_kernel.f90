...
!$xmp save_desc :: p, t, xarea,yarea,celly,celldy,volume,density0,energy0,pressure,viscosity_a,soundspeed,xvel0,yvel0,dt_min
    ...
    jk_control=1.1
!$ACC DATA    &
!$ACC PCOPY(celldx,celldy,cellx,celly,density0,soundspeed,viscosity_a,volume) &
!$ACC PCOPY(xarea,xvel0,yarea,yvel0,dt_min) &
!$ACC COPYIN(g_small)

!$xmp loop (j,k) on t(j,k)
!$ACC KERNELS LOOP INDEPENDENT REDUCTION(MIN : dt_min_val)
    DO k=y_min,y_max
