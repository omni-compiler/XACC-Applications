...
!$xmp shadow (2:3, 2:3) :: xvel0, yvel0
    ...
    press=0.0
!$ACC DATA &
!$ACC PCOPY(volume,density0,energy0,pressure,xvel0,yvel0)

!$xmp loop (j,k) on t(j,k)
!$ACC KERNELS LOOP INDEPENDENT REDUCTION(+:vol,mass,press,ie,ke) private(vsqrd,cell_vol,cell_mass,jv,kv) GANG(128)
    DO k=y_min,y_max
