...
contains 
  ...
  subroutine dynstep
    ...
    use mod_trcadv_thuburn, only: &
       src_update_tracer, &
       OPRT_divergence2_alloc, &  ! for openacc
       local_t_var                !
    ...
    !$acc data &
    !$acc& create(g_TEND,g_TENDq,f_TEND,f_TENDq,PROG0,PROGq0,PROG,PROGq,PROG_split,v_mean_c) &
    !$acc& create(rho,vx,vy,vz,w,ein,q,eth,pre,tem,th,rhogd,pregd,temd,qd,cv)  &
    ...
    if ( TRC_ADV_TYPE == 'DEFAULT' ) then
       ...
       !$acc kernels pcopy(PROGq0) pcopyin(PROGq) 
       PROGq0   (:,:,:,:) = PROGq   (:,:,:,:)
       !$acc end kernels
       PROGq0_pl(:,:,:,:) = PROGq_pl(:,:,:,:)
    endif
    ...
    do nl = 1, num_of_iteration_lstep
       ...
       !$acc kernels pcopy(rho,vx,vy,vz,ein) &
       !$acc& pcopyin(PROG, VMTR_GSGAM2) 
...
!        !$acc end loop
       ...
       !$acc kernels pcopy(cv,qd,tem,pre) pcopyin(q,ein,rho, CVW) 
       do l = 1, ADM_lall
       do k = 1, ADM_kall
       do g = 1, ADM_gall
          cv(g,k,l)  = 0.D0
          ...
          enddo
!           !$acc end loop
          cv(g,k,l) = cv(g,k,l) + qd(g,k,l) * CNST_CV

          tem(g,k,l) = ein(g,k,l) / cv(g,k,l)
          pre(g,k,l) = rho(g,k,l) * tem(g,k,l) * ( qd(g,k,l)*CNST_RAIR + q(g,k,l,I_QV)*CNST_RVAP )
       enddo
       enddo
       enddo
       !$acc end kernels

       !$acc kernels pcopy(w) pcopyin(PROG,rho, GRD_afac,GRD_bfac, VMTR_GSGAM2H) 
       do l = 1, ADM_lall
       ...
       !--- boundary conditions
       call bndcnd_all_lall( rho ,        & !--- [INOUT]
                           vx    ,        & !--- [INOUT]
                           vy    ,        & !--- [INOUT]
                           vz    ,        & !--- [INOUT]
                           w     ,        & !--- [INOUT]
       ...
       call thrmdyn_th_ldim( ADM_gall, ADM_lall, th(:,:,:), tem(:,:,:), pre(:,:,:) )
       call thrmdyn_eth_ldim( ADM_gall, ADM_lall, eth(:,:,:), ein(:,:,:), pre(:,:,:), rho(:,:,:) )

       !--- perturbations ( pred, rhod, temd )
       !$acc kernels pcopy(pregd,rhogd,temd) &
       !$acc& pcopyin(pre,pre_bs,rho,rho_bs,tem,tem_bs, VMTR_GSGAM2) 
       ...
       if ( NDIFF_LOCATION == 'IN_LARGE_STEP' ) then
          ...
          if ( nl == 1 ) then ! only first step
             !$acc kernels pcopy(f_TEND,f_TENDq) 
             f_TEND    (:,:,:,:) = 0.D0
             f_TENDq   (:,:,:,:) = 0.D0
             !$acc end kernels
             f_TEND_pl (:,:,:,:) = 0.D0
             ...
             if ( NUMFILTER_DOverticaldiff ) then ! numerical diffusion (vertical)
                write(*,*) "numerical diffusion (vertical) is not supported yet."
                stop
             ...
             if ( NUMFILTER_DOrayleigh ) then ! rayleigh damping
                write(*,*) "rayleigh damping is not supported yet."
                stop
       ...
       if ( TB_TYPE == 'SMG' ) then
          write(*,*) "Smagorinksy-type SGS model is not supported yet."
          stop
       ...
       if ( FLAG_NUDGING ) then
          write(*,*) "Nudging routines are not supported yet."
          stop
       ...
       call DEBUG_rapend  ('+++Small step')
          ...
          !$acc kernels pcopy(v_mean_c) pcopyin(PROG0) 
          v_mean_c(:,:,:,I_rhog)  = PROG0(:,:,:,I_rhog)
          v_mean_c(:,:,:,I_rhogvx)= PROG0(:,:,:,I_rhogvx)
          v_mean_c(:,:,:,I_rhogvy)= PROG0(:,:,:,I_rhogvy)
          v_mean_c(:,:,:,I_rhogvz)= PROG0(:,:,:,I_rhogvz)
          v_mean_c(:,:,:,I_rhogw) = PROG0(:,:,:,I_rhogw)
          !$acc end kernels

          v_mean_c_pl(:,:,:,I_rhog)  = PROG0_pl(:,:,:,I_rhog)  
          v_mean_c_pl(:,:,:,I_rhogvx)= PROG0_pl(:,:,:,I_rhogvx)
          v_mean_c_pl(:,:,:,I_rhogvy)= PROG0_pl(:,:,:,I_rhogvy)
          v_mean_c_pl(:,:,:,I_rhogvz)= PROG0_pl(:,:,:,I_rhogvz)
          v_mean_c_pl(:,:,:,I_rhogw) = PROG0_pl(:,:,:,I_rhogw) 
       ...
       if ( TRC_ADV_TYPE == 'MIURA2004' ) then
          ...
          if ( nl == num_of_iteration_lstep ) then
             ...
             if ( TIME_INTEG_TYPE /= 'TRCADV' ) then ! update rhogq by viscosity
                !$acc kernels pcopy(PROGq) pcopyin(f_TENDq) 
                PROGq(:,:,:,:) = PROGq(:,:,:,:) + TIME_DTL * f_TENDq(:,:,:,:)
                !$acc end kernels
             endif

             !$acc kernels pcopy(PROGq) 
       ...
       elseif( TRC_ADV_TYPE == 'DEFAULT' ) then
          ...
          do nq = 1, TRC_VMAX
             ...
             !$acc kernels pcopy(PROGq) pcopyin(PROGq0,g_TENDq,f_TENDq) 
             PROGq(:,:,:,:) = PROGq0(:,:,:,:)                                                                   &
       ...
       if ( nl /= num_of_iteration_lstep ) then
          ...
          call COMM_data_transfer_ACC( PROG, PROG_pl )

          !$acc kernels pcopy(PROG) 
          do m = 1, 6
          do l = 1, ADM_lall
          do k = 1, ADM_kall
             PROG(suf(ADM_gall_1d,1),k,l,m) = PROG(suf(ADM_gmax+1,ADM_gmin),k,l,m)
             PROG(suf(1,ADM_gall_1d),k,l,m) = PROG(suf(ADM_gmin,ADM_gmax+1),k,l,m)
          enddo
          enddo
          enddo
          !$acc end kernels
    ...
    if ( trim(TIME_INTEG_TYPE) == 'TRCADV' ) then
       write(*,*) "TRCADV is not supported yet."
       stop
