...
module mod_numfilter
  ...
  real(8), public,  allocatable, save :: Kh_coef   (:,:,:)              ! horizontal diffusion coefficient at cell center
  ...
  real(8), public,  allocatable, save :: Kh_coef_lap1   (:,:,:)         ! Kh_coef but 1st order laplacian  ! for openacc
  ...
  real(8), public,  allocatable, save :: divdamp_coef   (:,:,:)         ! divergence damping coefficient at cell center
...
contains
  ...
  subroutine numfilter_hdiffusion( &
    ...
    integer :: g, k, l, nq, p, m
    !---------------------------------------------------------------------------

    call DEBUG_rapstart('++++numfilter_hdiffusion')

    !$acc data &
    !$acc& pcopy(frhog,frhogvx,frhogvy,frhogvz,frhogw,frhoge,frhogetot,frhogq) &
    !$acc& pcopyin(rho,vx,vy,vz,w,temd,q) &
    !$acc& create(KH_coef_h,KH_coef_lap1_h,vtmp,vtmp2,qtmp,qtmp2,vtmp_lap1,qtmp_lap1,wk,rhog,rhog_h,fact,kh_max) &
    !$acc& pcopyin(Kh_coef,Kh_coef_lap1) &
    !$acc& pcopyin(VMTR_GSGAM2) &
    !$acc& pcopyin(rho_bs)

    if ( hdiff_nonlinear ) then
       call height_factor_ACC( ADM_kall, GRD_gz(:), GRD_htop, ZD_hdiff_nl, fact(:) )

       !$acc kernels pcopy(kh_max) pcopyin(fact) 
       kh_max(:) = ( 1.D0 - fact(:) ) * Kh_coef_maxlim &
                 + (        fact(:) ) * Kh_coef_minlim
       !$acc end kernels
    endif

    !$acc kernels pcopy(rhog) pcopyin(rho, VMTR_GSGAM2) 
    rhog   (:,:,:) = rho   (:,:,:) * VMTR_GSGAM2   (:,:,:)
    !$acc end kernels
    rhog_pl(:,:,:) = rho_pl(:,:,:) * VMTR_GSGAM2_pl(:,:,:)

    !
    !$acc kernels pcopy(rhog_h) pcopyin(rho, GRD_afac,GRD_bfac, VMTR_GSGAM2H) 
    do l = 1, ADM_lall
    ...
    !$acc kernels pcopy(vtmp) pcopyin(vx,vy,vz,w,temd,rho,rho_bs) 
    vtmp   (:,:,:,1) = vx     (:,:,:)
    vtmp   (:,:,:,2) = vy     (:,:,:)
    vtmp   (:,:,:,3) = vz     (:,:,:)
    vtmp   (:,:,:,4) = w      (:,:,:)
    ...
    ! copy beforehand
    if ( NUMFILTER_DOhorizontaldiff_lap1 ) then
       !$acc kernels pcopy(vtmp_lap1) pcopyin(vtmp) 
       vtmp_lap1   (:,:,:,:) = vtmp   (:,:,:,:)
       !$acc end kernels
    ...
    do p = 1, lap_order_hdiff
       ...
       if ( p == lap_order_hdiff ) then
          ...
          if ( hdiff_nonlinear ) then
             !$acc kernels pcopy(KH_coef) pcopyin(vtmp,Kh_max) 
             do l = 1, ADM_lall
                do k = 1, ADM_kall
                do g = 1, ADM_gall
             ...
             !$acc kernels pcopy(KH_coef_h) pcopyin(KH_coef) 
             do l = 1, ADM_lall
             do k = ADM_kmin, ADM_kmax
             do g = 1, ADM_gall
                if ((k >= ADM_kmin+1) .and. (k <= ADM_kmax)) then
          ...
          !$acc kernels pcopy(wk) pcopyin(rhog,KH_coef) 
       ...
       !$acc kernels pcopy(vtmp) pcopyin(vtmp2) 
       vtmp   (:,:,:,:) = -vtmp2   (:,:,:,:)
       !$acc end kernels
       vtmp_pl(:,:,:,:) = -vtmp2_pl(:,:,:,:)

       call COMM_data_transfer_ACC( vtmp, vtmp_pl )
    ...
    if ( NUMFILTER_DOhorizontaldiff_lap1 ) then
       ...
       !$acc kernels pcopy(KH_coef_lap1_h) pcopyin(KH_coef_lap1) 
       ...
       !$acc kernels pcopy(wk) pcopyin(rhog,KH_coef_lap1) 
       ...
       !$acc kernels pcopy(vtmp_lap1) pcopyin(vtmp2) 
       vtmp_lap1   (:,:,:,:) = -vtmp2   (:,:,:,:)
       !$acc end kernels
       vtmp_lap1_pl(:,:,:,:) = -vtmp2_pl(:,:,:,:)

       call COMM_data_transfer_ACC( vtmp_lap1, vtmp_lap1_pl )

    else
       !$acc kernels pcopy(KH_coef_lap1_h) 
       KH_coef_lap1_h   (:,:,:) = 0.D0
       !$acc end kernels
       KH_coef_lap1_h_pl(:,:,:) = 0.D0

       !$acc kernels pcopy(vtmp_lap1) 
       vtmp_lap1   (:,:,:,:) = 0.D0
       !$acc end kernels
       vtmp_lap1_pl(:,:,:,:) = 0.D0
    endif
    ...
    !--- Update tendency
    !$acc kernels pcopy(frhogvx,frhogvy,frhogvz,frhogw,frhoge,frhogetot,frhog) &
    !$acc& pcopyin(vtmp,vtmp_lap1,rhog,rhog_h,KH_coef,KH_coef_h,KH_coef_lap1,KH_coef_lap1_h) 
    do l = 1, ADM_lall
    do k = ADM_kmin, ADM_kmax
    do g = 1, ADM_gall
    ...
    if ( TRC_ADV_TYPE /= 'MIURA2004' ) then 
       ...
       !$acc kernels pcopy(qtmp) pcopyin(q) 
       qtmp   (:,:,:,:) = q   (:,:,:,:)
       !$acc end kernels
       qtmp_pl(:,:,:,:) = q_pl(:,:,:,:)
       ...
       ! copy beforehand
       if ( NUMFILTER_DOhorizontaldiff_lap1 ) then
          !$acc kernels pcopy(qtmp_lap1) pcopyin(qtmp) 
          qtmp_lap1   (:,:,:,:) = qtmp   (:,:,:,:)
          !$acc end kernels
       ...
       do p = 1, lap_order_hdiff
          ...
          !$acc kernels pcopy(qtmp) pcopyin(qtmp2) 
          qtmp   (:,:,:,:) = -qtmp2   (:,:,:,:)
          !$acc end kernels
          qtmp_pl(:,:,:,:) = -qtmp2_pl(:,:,:,:)

          call COMM_data_transfer_ACC( qtmp, qtmp_pl )
       ...
       if ( NUMFILTER_DOhorizontaldiff_lap1 ) then
          ...
          !$acc kernels pcopy(qtmp_lap1) pcopyin(qtmp2) 
          qtmp_lap1   (:,:,:,:) = -qtmp2   (:,:,:,:)
          !$acc end kernels
          qtmp_lap1_pl(:,:,:,:) = -qtmp2_pl(:,:,:,:)

          call COMM_data_transfer_ACC( qtmp_lap1(:,:,:,:), qtmp_lap1_pl(:,:,:,:) )
       else
          !$acc kernels pcopy(qtmp_lap1) 
          qtmp_lap1   (:,:,:,:) = 0.D0
          !$acc end kernels
          qtmp_lap1_pl(:,:,:,:) = 0.D0
       endif

       !$acc kernels pcopy(frhogq) pcopyin(qtmp,qtmp_lap1) 
       do nq = 1, TRC_VMAX
       do l  = 1, ADM_lall
       do k  = ADM_kmin, ADM_kmax
       do g = 1, ADM_gall
          frhogq(g,k,l,nq) = frhogq(g,k,l,nq) - ( qtmp(g,k,l,nq) + qtmp_lap1(g,k,l,nq) )
       enddo
       enddo
       enddo
       enddo
  ...
  subroutine numfilter_divdamp( &
    ...
    !$acc& create(vtmp,vtmp2,cnv) &
    !$acc& pcopyin(divdamp_coef) &
    !$acc& pcopyin(GRD_rdgzh)
    ...
    if ( lap_order_divdamp > 1 ) then
       do p = 1, lap_order_divdamp-1
          ...
          call COMM_data_transfer_ACC( vtmp2, vtmp2_pl )
    ...
    !--- X coeffcient
    !$acc kernels pcopy(gdx,gdy,gdz) pcopyin(divdamp_coef,vtmp2) 
  ...
  subroutine numfilter_divdamp_2d( &
    ...
    !--- X coeffcient
    !$acc kernels pcopy(gdx,gdy,gdz) pcopyin(divdamp_2d_coef,vtmp2) 
  ...
  subroutine height_factor( &
    ...
    return
  end subroutine height_factor

  !
  subroutine height_factor_ACC( &
       kdim,          &
       z,             &
       z_top,         &
       z_bottomlimit, &
    ...
    !$acc kernels pcopy(factor) pcopyin(z) 
    do k = 1, kdim
       sw = 0.5D0 + sign( 0.5D0, z(k)-z_bottomlimit )
    ...
    return
  end subroutine height_factor_ACC
