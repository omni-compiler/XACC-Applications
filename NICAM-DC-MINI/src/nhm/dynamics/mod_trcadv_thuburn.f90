...
module mod_trcadv_thuburn
  ...
  real(8), public,  allocatable, save :: local_t_var(:,:,:,:,:)  ! for openacc
...
contains
  ...
  subroutine src_update_tracer( &
    ...
    real(8) :: flx_h   (ADM_gall,   ADM_kall,ADM_lall   ,6)    ! cnvtd flx_h
    real(8) :: flx_h_pl(ADM_gall_pl,ADM_kall,ADM_lall_pl)
    real(8) :: c       (ADM_gall,   ADM_kall,ADM_lall   ,6)    ! cnvtd c
    ...
    !$acc& create(rhog,rrhog,q,q_h,d,flx_v,ck,flx_h,c,vx_r,vy_r,vz_r,hdiv,cp) &
    !$acc& pcopyin(VMTR_GSGAMH,VMTR_RGSGAM2, VMTR_GZXH,VMTR_GZYH,VMTR_GZZH,VMTR_RGSH) &
    !$acc& pcopyin(GRD_afac,GRD_bfac,GRD_rdgz)

    !---------------------------------------------------------------------------
    ! Vertical Advection (fractioanl step) : 1st
    !---------------------------------------------------------------------------
    !$acc kernels pcopy(flx_v) &
    !$acc& pcopyin(rhogvx_mean,rhogvy_mean,rhogvz_mean,rhogw_mean) &
    !$acc& pcopyin(VMTR_GSGAMH,VMTR_RGSGAM2, VMTR_GZXH,VMTR_GZYH,VMTR_GZZH,VMTR_RGSH) &
    ...
    !$acc kernels pcopy(rrhog,d,ck) pcopyin(flx_v,rhog_in, GRD_rdgz) 
    do l = 1, ADM_lall
       do k = 1, ADM_kall
       do g = 1, ADM_gall
    ...
    do nq = 1, nqmax
       ...
       !$acc kernels pcopy(q) pcopyin(rhogq,rrhog) 
       do l = 1, ADM_lall
          do k = 1, ADM_kall
             do g = 1, ADM_gall
       ...
       !$acc kernels pcopy(q_h) pcopyin(q, GRD_afac,GRD_bfac) 
       do l = 1, ADM_lall
       ...
       !$acc kernels pcopy(rhogq) pcopyin(flx_v,q_h, GRD_rdgz) 
       do l = 1, ADM_lall
          do k = ADM_kmin, ADM_kmax
             do g = 1, ADM_gall
    ...
    g_d1 = suf(ADM_gall_1d,1)
    g_s1 = suf(ADM_gmax+1,ADM_gmin)

    g_d2 = suf(1,ADM_gall_1d)
    g_s2 = suf(ADM_gmin,ADM_gmax+1)

    !--- update rhog
    !$acc kernels pcopy(rhog) pcopyin(flx_v,rhog_in, GRD_rdgz) 
    do l = 1, ADM_lall
       do k = 1, ADM_kall
       do g = 1, ADM_gall
          if ((k >= ADM_kmin) .and. (k <= ADM_kmax)) then
             gg = g
             if ( g .eq. g_d1 ) gg = g_s1
             if ( g .eq. g_d2 ) gg = g_s2
             rhog(g,k,l) = rhog_in(gg,k,l)                                  &
                         - ( flx_v(gg,k+1,l) - flx_v(gg,k,l) ) * GRD_rdgz(k) &
                         + b1 * frhog(gg,k,l) * dt
          else if (k == ADM_kmin-1) then
             rhog(g,k,l) = rhog_in(g,k+1,l)
          else if (k == ADM_kmin+1) then
             rhog(g,k,l) = rhog_in(g,k-1,l)
          endif
       enddo
    ...
    !---------------------------------------------------------------------------
    !$acc kernels pcopy(vx_r,vy_r,vz_r, rhogvx_mean,rhogvy_mean,rhogvz_mean, VMTR_RGAM) 
    do l = 1, ADM_lall
       do k = 1, ADM_kall
          do g = 1, ADM_gall
    ...
    !$acc kernels pcopy(rrhog,d,c) pcopyin(rhog,frhog,flx_h) 
    do l = 1, ADM_lall
       do k = 1, ADM_kall
          do g = 1, ADM_gall
             ...
             !--- Courant number
             c(g,k,l,1) = flx_h(g,k,l,1) * rrhog(g,k,l)
             c(g,k,l,2) = flx_h(g,k,l,2) * rrhog(g,k,l)
             c(g,k,l,3) = flx_h(g,k,l,3) * rrhog(g,k,l)
             c(g,k,l,4) = flx_h(g,k,l,4) * rrhog(g,k,l)
             c(g,k,l,5) = flx_h(g,k,l,5) * rrhog(g,k,l)
             c(g,k,l,6) = flx_h(g,k,l,6) * rrhog(g,k,l)
          enddo
       enddo
    enddo
    !$acc end kernels
    ...
    do nq = 1, nqmax
       ...
       !--- update rhogq
       !$acc kernels pcopy(rhogq) pcopyin(hdiv) 
       do l = 1, ADM_lall
          do k = 1, ADM_kall
             do g = 1, ADM_gall
    ...
    do l = 1, ADM_lall
       ...
       do g = 1, ADM_gall
          if ((g >= nstart) .and. (g <= nend)) then
             n = g
             rhog(n,k,l)= rhog(n,k,l) - ( flx_h(n,k,l,1) &
                                        + flx_h(n,k,l,2) &
                                        + flx_h(n,k,l,3) &
                                        + flx_h(n,k,l,4) &
                                        + flx_h(n,k,l,5) &
          ...
          if ( g == g_s1 ) rhog(g_d1,k,l) = rhog(g,k,l)
          if ( g == g_s2 ) rhog(g_d2,k,l) = rhog(g,k,l)
       enddo
       enddo
    enddo
    !$acc end kernels
    ...
    !---------------------------------------------------------------------------
    !$acc kernels pcopy(rrhog,d) pcopyin(rhog,frhog) 
    do l = 1, ADM_lall
       do k = 1, ADM_kall
          do g = 1, ADM_gall
             rrhog(g,k,l) = 1.D0 / rhog(g,k,l)
    ...
    !---- Courant numbers at cell boundary
    !$acc kernels pcopy(ck) pcopyin(flx_v,rrhog,  GRD_rdgz) 
    do l = 1, ADM_lall
       do k = ADM_kmin-1, ADM_kmax+1
       do g = 1, ADM_gall
          if ((k >= ADM_kmin) .and. (k <= ADM_kmax)) then
  ...
  subroutine advlim_thuburn_v( &
    ...
    real(8) :: qin_min    (ADM_gall,   ADM_kall,ADM_lall,2)    ! cnvtd qin_min
    real(8) :: qin_min_pl (ADM_gall_pl,ADM_kall,2)
    real(8) :: qin_max    (ADM_gall,   ADM_kall,ADM_lall,2)    ! cnvtd qin_max
    real(8) :: qin_max_pl (ADM_gall_pl,ADM_kall,2)

    real(8) :: qout_min   (ADM_gall,   ADM_kall,ADM_lall)    ! cnvtd qout_min
    real(8) :: qout_min_pl(ADM_gall_pl,ADM_kall)
    real(8) :: qout_max   (ADM_gall,   ADM_kall,ADM_lall)    ! cnvtd qout_max
    ...
    !$acc& pcopy(q_h) &
    !$acc& pcopyin(q,ck,d) &
    !$acc& create(qin_min,qin_max,qout_min,qout_max)

    !--- inflow & outflow limiter
    !$acc kernels pcopy(qin_min,qin_max,qout_min,qout_max) pcopyin(d,q,ck) 
    do l = 1, ADM_lall
       do k = 1, ADM_kall
          do n = 1, ADM_gall
             qin_min(n,k,l,1) = CNST_MAX_REAL
             qin_max(n,k,l,1) =-CNST_MAX_REAL
             if (( k >= ADM_kmin ) .and. ( k <= ADM_kmax+1 )) then
                if ( ck(n,k,l,1) <= 0.D0 ) then
                   qin_min(n,k,l,1) = min( q(n,k,l), q(n,k-1,l) )
                   qin_max(n,k,l,1) = max( q(n,k,l), q(n,k-1,l) )
                endif
             endif

             qin_min(n,k,l,2) = CNST_MAX_REAL
             qin_max(n,k,l,2) =-CNST_MAX_REAL
             if (( k >= ADM_kmin-1 ) .and. ( k <= ADM_kmax )) then
                if ( ck(n,k+1,l,1) > 0.D0 ) then
                   qin_min(n,k,l,2) = min( q(n,k+1,l), q(n,k,l) )
                   qin_max(n,k,l,2) = max( q(n,k+1,l), q(n,k,l) )
                endif
             endif

             !

             q_mp1_min = min( qin_min(n,k,l,1), qin_min(n,k,l,2) )
             if( q_mp1_min ==  CNST_MAX_REAL ) q_mp1_min = q(n,k,l)
             q_mp1_min = max( 0.D0, q_mp1_min )

             q_mp1_max = max( qin_max(n,k,l,1), qin_max(n,k,l,2) )
             ...
             c_qin_max = ( mask1 ) * ( ck(n,k,l,1) * qin_max(n,k,l,1) ) &
                       + ( mask2 ) * ( ck(n,k,l,2) * qin_max(n,k,l,2) )
             c_qin_min = ( mask1 ) * ( ck(n,k,l,1) * qin_min(n,k,l,1) ) &
                       + ( mask2 ) * ( ck(n,k,l,2) * qin_min(n,k,l,2) )

             if ( abs(c_out) <= CNST_EPS_ZERO ) then
                qout_min(n,k,l) = q(n,k,l)
                qout_max(n,k,l) = q(n,k,l)
             else
                qout_min(n,k,l) = ( q(n,k,l)                                &
                                 - c_qin_max                                &
    ...
    !--- apply limiter
    !$acc kernels pcopy(q_h) pcopyin(qin_min,qin_max,qout_min,qout_max,ck) 
    do l = 1, ADM_lall
       do k = ADM_kmin, ADM_kmax+1
          do n = 1, ADM_gall
             mask = 0.5D0 - sign( 0.5D0, ck(n,k,l,1) )

             tmp = (        mask ) * min( max( q_h(n,k,l), qin_min(n,k  ,l,1) ), qin_max(n,k  ,l,1) ) &
                 + ( 1.D0 - mask ) * min( max( q_h(n,k,l), qin_min(n,k-1,l,2) ), qin_max(n,k-1,l,2) )

             q_h(n,k,l) = (        mask ) * max( min( tmp, qout_max(n,k-1,l) ), qout_min(n,k-1,l) ) &
                        + ( 1.D0 - mask ) * max( min( tmp, qout_max(n,k  ,l) ), qout_min(n,k  ,l) )
          enddo
       enddo
    enddo
    !$acc end kernels
  ...
  !-----------------------------------------------------------------------------
  subroutine OPRT_divergence2_alloc( )
    use mod_adm, only :   &
         ADM_TI,          &
         ADM_TJ,          &
         ADM_kall,        &
  ...
  end subroutine OPRT_divergence2_alloc
  ...
  subroutine OPRT_divergence2_prep( &
    ...
    real(8), intent(out) :: flx_h(ADM_gall,ADM_kall,ADM_lall,6)    ! cnvtd flx_h
    ...
    !$acc data &
    !$acc& pcopy(flx_h,cp) &
    !$acc& pcopyin(rhovx,rhovy,rhovz,rho) &
    !$acc& create(rhovxt,rhovyt,rhovzt,rhot) &
    !$acc& pcopyin(local_t_var) &
    !$acc& pcopyin(GMTR_T_var,GMTR_A_var,GMTR_P_var) &
    !$acc& pcopyin(GRD_xt) &
    !$acc& pcopyin(ADM_prc_tab,ADM_rgn_vnum)

    k0 = ADM_KNONE

    !$acc kernels pcopy(flx_h) 
    flx_h   (:,:,:,:)   = 0.D0
    !$acc end kernels

    !$acc kernels pcopy(cp) 
    cp      (:,:,:,:,:) = 0.D0
    !$acc end kernels
    ...
    nend   = suf(ADM_gmax  ,ADM_gmax  )
    !$acc kernels pcopy(rhovxt,rhovyt,rhovzt,rhot) &
    !$acc& pcopyin(rhovx,rhovy,rhovz,rho,local_t_var, GMTR_T_var) 
    do l = 1, ADM_lall
       do k = 1, ADM_kall
          do n = nstart, nend
...
!                 *rho(n+1+ADM_gall_1d,k,l)
             rhot(n,k,l,ADM_TJ) = local_T_var(n,k0,l,ADM_TJ,GMTR_T_W1) *rho(n,k,l) &
                                + local_T_var(n,k0,l,ADM_TJ,GMTR_T_W2) *rho(n+1+ADM_gall_1d,k,l) &
                                + local_T_var(n,k0,l,ADM_TJ,GMTR_T_W3) *rho(n+ADM_gall_1d,k,l)
...
!!$                  *rho(n+ADM_gall_1d,k,l)
    ...
    !$acc kernels pcopy(rhovxt,rhovyt,rhovzt,rhot) pcopyin(ADM_prc_tab,ADM_rgn_vnum) 
    do l = 1, ADM_lall
       do k = 1, ADM_kall
...
!!$                     *rho_pl(np1(n),k,l)
    ...
    nend   = suf(ADM_gmax,  ADM_gmax  )
    !$acc kernels pcopy(cp,flx_h) &
    !$acc& pcopyin(rhovxt,rhovyt,rhovzt,rhot, GRD_xt, GMTR_A_var,GMTR_P_var) 
    do l=1,ADM_lall
       do k = 1, ADM_kall
          do n = nstart,nend
             rpx = 0.5D0 * ( GRD_xt(n,k0,l,ADM_TI,GRD_XDIR) + GRD_xt(n-ADM_gall_1d,k0,l,ADM_TJ,GRD_XDIR) )
             rpy = 0.5D0 * ( GRD_xt(n,k0,l,ADM_TI,GRD_YDIR) + GRD_xt(n-ADM_gall_1d,k0,l,ADM_TJ,GRD_YDIR) )
             rpz = 0.5D0 * ( GRD_xt(n,k0,l,ADM_TI,GRD_ZDIR) + GRD_xt(n-ADM_gall_1d,k0,l,ADM_TJ,GRD_ZDIR) )

             rhovxa = 0.5D0 * ( rhovxt(n-ADM_gall_1d,k,l,ADM_TJ) + rhovxt(n,k,l,ADM_TI) )
             rhovya = 0.5D0 * ( rhovyt(n-ADM_gall_1d,k,l,ADM_TJ) + rhovyt(n,k,l,ADM_TI) )
             rhovza = 0.5D0 * ( rhovzt(n-ADM_gall_1d,k,l,ADM_TJ) + rhovzt(n,k,l,ADM_TI) )
             rhoa   = 0.5D0 * ( rhot  (n-ADM_gall_1d,k,l,ADM_TJ) + rhot  (n,k,l,ADM_TI) )

             cp(n,k,l,ADM_AI,GRD_XDIR) = rpx - rhovxa / rhoa * dt * 0.5D0
             cp(n,k,l,ADM_AI,GRD_YDIR) = rpy - rhovya / rhoa * dt * 0.5D0
             cp(n,k,l,ADM_AI,GRD_ZDIR) = rpz - rhovza / rhoa * dt * 0.5D0

             ccc = ( rhovxa * GMTR_A_var(n,k0,l,ADM_AI,GMTR_A_HNX) &
                   + rhovya * GMTR_A_var(n,k0,l,ADM_AI,GMTR_A_HNY) &
                   + rhovza * GMTR_A_var(n,k0,l,ADM_AI,GMTR_A_HNZ) )

             flx_h(n  ,k,l,1) =  ccc * dt * GMTR_P_var(n  ,k0,l,GMTR_P_RAREA)    ! cnvtd flx_h
             flx_h(n+1,k,l,4) = -ccc * dt * GMTR_P_var(n+1,k0,l,GMTR_P_RAREA)    ! cnvtd flx_h
          enddo
       enddo
    enddo
    !$acc end kernels
    ...
    do l=1,ADM_lall
       do k = 1, ADM_kall
          do n = nstart,nend
             rpx = 0.5D0*(GRD_xt(n,k0,l,ADM_TI,GRD_XDIR)+GRD_xt(n,k0,l,ADM_TJ,GRD_XDIR))
             rpy = 0.5D0*(GRD_xt(n,k0,l,ADM_TI,GRD_YDIR)+GRD_xt(n,k0,l,ADM_TJ,GRD_YDIR))
             rpz = 0.5D0*(GRD_xt(n,k0,l,ADM_TI,GRD_ZDIR)+GRD_xt(n,k0,l,ADM_TJ,GRD_ZDIR))

             rhovxa = (rhovxt(n,k,l,ADM_TI)+rhovxt(n,k,l,ADM_TJ))*0.5D0
             rhovya = (rhovyt(n,k,l,ADM_TI)+rhovyt(n,k,l,ADM_TJ))*0.5D0
             rhovza = (rhovzt(n,k,l,ADM_TI)+rhovzt(n,k,l,ADM_TJ))*0.5D0
             rhoa   = (rhot  (n,k,l,ADM_TI)+rhot  (n,k,l,ADM_TJ))*0.5D0
             !
             cp(n,k,l,ADM_AIJ,GRD_XDIR) = rpx - rhovxa/rhoa*dt*0.5D0
             cp(n,k,l,ADM_AIJ,GRD_YDIR) = rpy - rhovya/rhoa*dt*0.5D0
             cp(n,k,l,ADM_AIJ,GRD_ZDIR) = rpz - rhovza/rhoa*dt*0.5D0
             !
             ccc = &
                  (rhovxa*GMTR_A_var(n,k0,l,ADM_AIJ,GMTR_A_HNX)&
                  +rhovya*GMTR_A_var(n,k0,l,ADM_AIJ,GMTR_A_HNY)&
                  +rhovza*GMTR_A_var(n,k0,l,ADM_AIJ,GMTR_A_HNZ))
             flx_h(n              ,k,l,2) = ccc*dt*GMTR_P_var(n,k0,l,GMTR_P_RAREA)    ! cnvtd flx_h
             flx_h(n+1+ADM_gall_1d,k,l,5) =-ccc*dt*GMTR_P_var(n+1+ADM_gall_1d,k0,l,GMTR_P_RAREA)    ! cnvtd flx_h
             !
          enddo
       enddo
    enddo
    ...
    nend   = suf(ADM_gmax  ,ADM_gmax  )
    !$acc kernels pcopy(cp,flx_h) &
    !$acc& pcopyin(rhovxt,rhovyt,rhovzt,rhot) &
    !$acc& pcopyin(GRD_xt, GMTR_A_var,GMTR_P_var, ADM_prc_tab,ADM_rgn_vnum) 
    do l=1,ADM_lall
       do k = 1, ADM_kall
          do n = nstart,nend
             rpx = 0.5D0*(GRD_xt(n,k0,l,ADM_TJ,GRD_XDIR)+GRD_xt(n-1,k0,l,ADM_TI,GRD_XDIR))
             rpy = 0.5D0*(GRD_xt(n,k0,l,ADM_TJ,GRD_YDIR)+GRD_xt(n-1,k0,l,ADM_TI,GRD_YDIR))
             rpz = 0.5D0*(GRD_xt(n,k0,l,ADM_TJ,GRD_ZDIR)+GRD_xt(n-1,k0,l,ADM_TI,GRD_ZDIR))
             rhovxa = (rhovxt(n,k,l,ADM_TJ)+rhovxt(n-1,k,l,ADM_TI))*0.5D0
             rhovya = (rhovyt(n,k,l,ADM_TJ)+rhovyt(n-1,k,l,ADM_TI))*0.5D0
             rhovza = (rhovzt(n,k,l,ADM_TJ)+rhovzt(n-1,k,l,ADM_TI))*0.5D0
             rhoa   = (rhot  (n,k,l,ADM_TJ)+rhot  (n-1,k,l,ADM_TI))*0.5D0
             !
             cp(n,k,l,ADM_AJ,GRD_XDIR) = rpx - rhovxa/rhoa*dt*0.5D0
             cp(n,k,l,ADM_AJ,GRD_YDIR) = rpy - rhovya/rhoa*dt*0.5D0
             cp(n,k,l,ADM_AJ,GRD_ZDIR) = rpz - rhovza/rhoa*dt*0.5D0
             !
             ccc = &
                  (rhovxa*GMTR_A_var(n,k0,l,ADM_AJ,GMTR_A_HNX)&
                  +rhovya*GMTR_A_var(n,k0,l,ADM_AJ,GMTR_A_HNY)&
                  +rhovza*GMTR_A_var(n,k0,l,ADM_AJ,GMTR_A_HNZ))
             flx_h(n            ,k,l,3) = ccc*dt*GMTR_P_var(n,k0,l,GMTR_P_RAREA)    ! cnvtd flx_h
             flx_h(n+ADM_gall_1d,k,l,6) =-ccc*dt*GMTR_P_var(n+ADM_gall_1d,k0,l,GMTR_P_RAREA)    ! cnvtd flx_h

             !
             if ( n+ADM_gall_1d == suf(ADM_gmin,ADM_gmin) ) then
                rgnid=ADM_prc_tab(l,ADM_prc_me)
                if(ADM_rgn_vnum(ADM_W,rgnid)==3) then
                   flx_h(n+ADM_gall_1d,k,l,6) = 0.0D0
                end if
             endif
          enddo
       enddo
  ...
  subroutine OPRT_divergence2( &
    ...
    real(8), intent(in)  :: flx_h   (ADM_gall,ADM_kall,ADM_lall,6)    ! cnvtd flx_h
    real(8), intent(in)  :: flx_h_pl(  ADM_gall_pl,ADM_kall,ADM_lall_pl)
    real(8), intent(in)  :: c       (ADM_gall,ADM_kall,ADM_lall,6)    ! cnvtd c
    ...
    real(8)  :: sa(ADM_gall,ADM_kall,ADM_lall,ADM_AI:ADM_AJ)    ! cnvtd sa
    real(8)  :: sa_pl(ADM_gall_pl,ADM_kall,ADM_lall_pl)
    real(8)  :: sa_p,sa_m

    real(8)  :: sw(ADM_gall,ADM_kall,ADM_lall,ADM_AI:ADM_AJ)    ! cnvtd sw

    real(8)  :: s_in_min(ADM_gall,ADM_kall,ADM_lall,6)    ! cnvtd s_in_min
    real(8)  :: s_in_min_pl(ADM_gall_pl,ADM_kall,ADM_lall_pl,2)
    real(8)  :: s_in_max(ADM_gall,ADM_kall,ADM_lall,6)    ! cnvtd s_in_max
...
!    logical :: NON_NEG=.false.
    ...
    !
    integer :: nstart,nend
    integer :: nstartm,nendm
    integer :: nstart13,nend13
    integer :: nstart25,nend25
    integer :: nstart36,nend36
    ...
    !$acc data &
    !$acc& pcopy(scl) &
    !$acc& pcopyin(s,flx_h,c,cp,d) &
    !$acc& create(sa,sw,s_in_min,s_in_max,wrk) &
    !$acc& pcopyin(ADM_prc_tab,ADM_rgn_vnum) &
    !$acc& pcopyin(GRD_x)
    ...
    !$acc kernels pcopy(wrk) pcopyin(s) 
    do l = 1, ADM_lall
       do k = 1, ADM_kall
          do n = 1, ADM_gall
...
!    do n = 1, ADM_gall
!    do m = 1, 6
!       s_in_min(n,k,l,m) = CNST_MAX_REAL    ! cnvtd s_in_min
!    enddo
!    enddo
!    enddo
!    enddo
...
!    do n = 1, ADM_gall
!    do m = 1, 6
!       s_in_max(n,k,l,m) = -CNST_MAX_REAL    ! cnvtd s_in_max
!    enddo
!    enddo
!    enddo
!    enddo

    !$acc kernels pcopy(sw) pcopyin(c) 
    do l = 1, ADM_lall
       do k = 1, ADM_kall
          do n = 1, ADM_gall
             sw(n,k,l,ADM_AI ) = 0.D0
             sw(n,k,l,ADM_AIJ) = 0.D0
             sw(n,k,l,ADM_AJ ) = 0.D0
             if ( c(n,k,l,1) <= 0.D0 ) sw(n,k,l,ADM_AI ) = 1.D0    ! cnvtd c sw
             if ( c(n,k,l,2) <= 0.D0 ) sw(n,k,l,ADM_AIJ) = 1.D0    ! cnvtd c sw
             if ( c(n,k,l,3) <= 0.D0 ) sw(n,k,l,ADM_AJ ) = 1.D0    ! cnvtd c sw
          enddo
       enddo
    enddo
    !$acc end kernels

    nstartm = suf(ADM_gmin,ADM_gmin)
    nendm   = suf(ADM_gmax,ADM_gmax)

    nstart13 = suf(ADM_gmin-1,ADM_gmin  )
    nend13   = suf(ADM_gmin-1,ADM_gmin  )

    nstart25 = suf(ADM_gmin-1,ADM_gmin-1)
    nend25   = suf(ADM_gmin-1,ADM_gmin  )

    nstart36 = suf(ADM_gmin,  ADM_gmin-1)
    nend36   = suf(ADM_gmin-1,ADM_gmin  )

    nstart = min( nstartm, nstart13, nstart25, nstart36 )
    nend   = max( nendm,   nend13,   nend25,   nend36   )

    !$acc kernels pcopy(s_in_min,s_in_max) pcopyin(s,sw, ADM_prc_tab,ADM_rgn_vnum) 
    do l = 1, ADM_lall
       do k = 1, ADM_kall
          do n = nstart, nend
             ij     = n
             ...
             if ((ADM_rgn_vnum(ADM_W,rgnid) == 3) .and. (n == nstart25)) then
             ...
             if ((( n >= nstartm ) .and. ( n <= nendm )) .or. (( n >= nstart13 ) .and. ( n <= nend13 ))) then
                s_in_min(ij,    k,l,1) = (        sw(n,k,l,ADM_AI ) ) * AI_min        &
                                       + ( 1.D0 - sw(n,k,l,ADM_AI ) ) * CNST_MAX_REAL
                s_in_min(ip1j,  k,l,4) = (        sw(n,k,l,ADM_AI ) ) * CNST_MAX_REAL &
                                       + ( 1.D0 - sw(n,k,l,ADM_AI ) ) * AI_min
                s_in_max(ij,    k,l,1) = (        sw(n,k,l,ADM_AI ) ) * AI_max         &
                                       + ( 1.D0 - sw(n,k,l,ADM_AI ) ) * (-CNST_MAX_REAL)
                s_in_max(ip1j,  k,l,4) = (        sw(n,k,l,ADM_AI ) ) * (-CNST_MAX_REAL) &
                                       + ( 1.D0 - sw(n,k,l,ADM_AI ) ) * AI_max
             endif

             if ((( n >= nstartm ) .and. ( n <= nendm )) .or. (( n >= nstart25 ) .and. ( n <= nend25 ))) then
                s_in_min(ij,    k,l,2) = (        sw(n,k,l,ADM_AIJ) ) * AIJ_min       &
                                       + ( 1.D0 - sw(n,k,l,ADM_AIJ) ) * CNST_MAX_REAL
                s_in_min(ip1jp1,k,l,5) = (        sw(n,k,l,ADM_AIJ) ) * CNST_MAX_REAL &
                                       + ( 1.D0 - sw(n,k,l,ADM_AIJ) ) * AIJ_min
                s_in_max(ij,    k,l,2) = (        sw(n,k,l,ADM_AIJ) ) * AIJ_max        &
                                       + ( 1.D0 - sw(n,k,l,ADM_AIJ) ) * (-CNST_MAX_REAL)
                s_in_max(ip1jp1,k,l,5) = (        sw(n,k,l,ADM_AIJ) ) * (-CNST_MAX_REAL) &
                                       + ( 1.D0 - sw(n,k,l,ADM_AIJ) ) * AIJ_max
             endif

             if ((( n >= nstartm ) .and. ( n <= nendm )) .or. (( n >= nstart36 ) .and. ( n <= nend36 ))) then
                s_in_min(ij,    k,l,3) = (        sw(n,k,l,ADM_AJ ) ) * AJ_min        &
                                       + ( 1.D0 - sw(n,k,l,ADM_AJ ) ) * CNST_MAX_REAL
                s_in_min(ijp1,  k,l,6) = (        sw(n,k,l,ADM_AJ ) ) * CNST_MAX_REAL &
                                       + ( 1.D0 - sw(n,k,l,ADM_AJ ) ) * AJ_min
                s_in_max(ij,    k,l,3) = (        sw(n,k,l,ADM_AJ ) ) * AJ_max         &
                                       + ( 1.D0 - sw(n,k,l,ADM_AJ ) ) * (-CNST_MAX_REAL)
                s_in_max(ijp1,  k,l,6) = (        sw(n,k,l,ADM_AJ ) ) * (-CNST_MAX_REAL) &
                                       + ( 1.D0 - sw(n,k,l,ADM_AJ ) ) * AJ_max
             endif
          enddo
       enddo
    enddo
    ...
    nstart = suf(ADM_gmin, ADM_gmin )
    nend   = suf(ADM_gmax ,ADM_gmax )
    !$acc kernels pcopy(wrk) pcopyin(c,d,s,s_in_min,s_in_max) 
    do l=1,ADM_lall
       do k = 1, ADM_kall
       do n = nstart,nend
          s_m1_k_min_n = min(s_in_min(n,k,l,1),s_in_min(n,k,l,2),s_in_min(n,k,l,3),&    ! cnvtd s_in_min s_in_min s_in_min
                             s_in_min(n,k,l,4),s_in_min(n,k,l,5),s_in_min(n,k,l,6))    ! cnvtd s_in_min s_in_min s_in_min
          if(s_m1_k_min_n==CNST_MAX_REAL) s_m1_k_min_n = s(n,k,l)
          s_m1_k_max_n = max(s_in_max(n,k,l,1),s_in_max(n,k,l,2),s_in_max(n,k,l,3),&    ! cnvtd s_in_max s_in_max s_in_max
                             s_in_max(n,k,l,4),s_in_max(n,k,l,5),s_in_max(n,k,l,6))    ! cnvtd s_in_max s_in_max s_in_max
          if(s_m1_k_max_n==-CNST_MAX_REAL) s_m1_k_max_n = s(n,k,l)

          c_in_sum_n &
            = (0.5D0-sign(0.5D0,c(n,k,l,1)))*c(n,k,l,1)&    ! cnvtd c c
            + (0.5D0-sign(0.5D0,c(n,k,l,2)))*c(n,k,l,2)&    ! cnvtd c c
            + (0.5D0-sign(0.5D0,c(n,k,l,3)))*c(n,k,l,3)&    ! cnvtd c c
            + (0.5D0-sign(0.5D0,c(n,k,l,4)))*c(n,k,l,4)&    ! cnvtd c c
            + (0.5D0-sign(0.5D0,c(n,k,l,5)))*c(n,k,l,5)&    ! cnvtd c c
            + (0.5D0-sign(0.5D0,c(n,k,l,6)))*c(n,k,l,6)    ! cnvtd c c
          ...
          c_qin_sum_max_n &
            = (0.5D0-sign(0.5D0,c(n,k,l,1)))*(c(n,k,l,1)*s_in_max(n,k,l,1))&    ! cnvtd c c s_in_max
            + (0.5D0-sign(0.5D0,c(n,k,l,2)))*(c(n,k,l,2)*s_in_max(n,k,l,2))&    ! cnvtd c c s_in_max
            + (0.5D0-sign(0.5D0,c(n,k,l,3)))*(c(n,k,l,3)*s_in_max(n,k,l,3))&    ! cnvtd c c s_in_max
            + (0.5D0-sign(0.5D0,c(n,k,l,4)))*(c(n,k,l,4)*s_in_max(n,k,l,4))&    ! cnvtd c c s_in_max
            + (0.5D0-sign(0.5D0,c(n,k,l,5)))*(c(n,k,l,5)*s_in_max(n,k,l,5))&    ! cnvtd c c s_in_max
            + (0.5D0-sign(0.5D0,c(n,k,l,6)))*(c(n,k,l,6)*s_in_max(n,k,l,6))    ! cnvtd c c s_in_max

          c_qin_sum_min_n &
            = (0.5D0-sign(0.5D0,c(n,k,l,1)))*(c(n,k,l,1)*s_in_min(n,k,l,1))&    ! cnvtd c c s_in_min
            + (0.5D0-sign(0.5D0,c(n,k,l,2)))*(c(n,k,l,2)*s_in_min(n,k,l,2))&    ! cnvtd c c s_in_min
            + (0.5D0-sign(0.5D0,c(n,k,l,3)))*(c(n,k,l,3)*s_in_min(n,k,l,3))&    ! cnvtd c c s_in_min
            + (0.5D0-sign(0.5D0,c(n,k,l,4)))*(c(n,k,l,4)*s_in_min(n,k,l,4))&    ! cnvtd c c s_in_min
            + (0.5D0-sign(0.5D0,c(n,k,l,5)))*(c(n,k,l,5)*s_in_min(n,k,l,5))&    ! cnvtd c c s_in_min
            + (0.5D0-sign(0.5D0,c(n,k,l,6)))*(c(n,k,l,6)*s_in_min(n,k,l,6))    ! cnvtd c c s_in_min
...
1000 continue
    ...
    call COMM_data_transfer_ACC(wrk,wrk_pl)

    !--- basic scheme
    nstart = suf(ADM_gmin-1,ADM_gmin-1)
    nend   = suf(ADM_gmax,  ADM_gmax  )
    !$acc kernels pcopy(sa) pcopyin(c,s,cp,wrk, GRD_x) 
    !$acc loop gang
    do l=1,ADM_lall
    ...
    nstart = suf(ADM_gmin-1,ADM_gmin-1)
    nend   = suf(ADM_gmax  ,ADM_gmax  )
    !$acc kernels pcopy(sa) pcopyin(c,s_in_min,s_in_max,wrk) 
    !$acc loop gang
    do l=1,ADM_lall
       !$acc loop gang vector(4)
       do k = 1, ADM_kall
...
2000 continue
    ...
    nstart = suf(ADM_gmin  ,ADM_gmin  )
    nend   = suf(ADM_gmax  ,ADM_gmax  )
    !$acc kernels pcopy(scl) pcopyin(flx_h,sa) 
    !$acc loop gang
    do l=1,ADM_lall
       !$acc loop gang vector(4)
       do k = 1, ADM_kall
