...
module mod_af_heldsuarez
  ...
  public :: af_heldsuarez_lall   ! for openacc
...
contains
  ...
  subroutine af_heldsuarez_lall( &
    ...
    integer, intent(in)  :: ijdim 
    real(8), intent(in)  :: lat(ijdim,ldim)
    real(8), intent(in)  :: pre(ijdim,kdim,ldim)
    real(8), intent(in)  :: tem(ijdim,kdim,ldim)
    real(8), intent(in)  :: vx (ijdim,kdim,ldim)
    real(8), intent(in)  :: vy (ijdim,kdim,ldim)
    real(8), intent(in)  :: vz (ijdim,kdim,ldim)
    real(8), intent(out) :: fvx(ijdim,kdim,ldim)
    real(8), intent(out) :: fvy(ijdim,kdim,ldim)
    real(8), intent(out) :: fvz(ijdim,kdim,ldim)
    real(8), intent(out) :: fw (ijdim,kdim,ldim)
    real(8), intent(out) :: fe (ijdim,kdim,ldim)
    ...
    !$acc data pcopy(fvx,fvy,fvz,fw,fe) pcopyin(lat,pre,tem,vx,vy,vz)
    ...
    !$acc kernels pcopy(fvx,fvy,fvz,fe) pcopyin(lat,pre,vx,vy,vz,tem)
    !$acc loop gang
    do l = 1,    ldim
    ...
    do n = 1,    ijdim
       asl = abs( sin(lat(n,l)) )
       acl = abs( cos(lat(n,l)) )
       ap0 = abs( pre(n,k,l) / PRE00 )

       T_eq = (315.D0 - DT_y*asl*asl - Dth_z*log(ap0)*acl*acl ) * ap0**KAPPA
       T_eq = max(T_eq,T_eq2)

       sigma    = pre(n,k,l) / ( 0.5D0 * ( pre(n,kmin,l) + pre(n,kmin-1,l) ) )
       fact_sig = max( 0.D0, (sigma-sigma_b) / (1.D0-sigma_b) )

       k_v = k_f * fact_sig

       fvx(n,k,l) = -k_v * vx(n,k,l)
       fvy(n,k,l) = -k_v * vy(n,k,l)
       fvz(n,k,l) = -k_v * vz(n,k,l)

       k_t = k_a + ( k_s-k_a ) * fact_sig * acl**4

       fe(n,k,l) = -k_t * CV * ( tem(n,k,l)-T_eq )
    enddo
!     !$acc end loop
    enddo
!     !$acc end loop
...
!     !$acc end loop
  ...
  end subroutine af_heldsuarez_lall
