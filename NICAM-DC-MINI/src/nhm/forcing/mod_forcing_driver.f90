...
contains
  ...
  subroutine forcing
    ...
    use mod_af_heldsuarez, only: &
       af_HeldSuarez, &
       af_HeldSuarez_lall  ! for openacc
    ...
    !$acc& create(rhog,rhogvx,rhogvy,rhogvz,rhogw,rhoge,rhogq,rho,pre,tem,vx,vy,vz,w,q) &
    !$acc& create(fvx,fvy,fvz,fw,fe,fq) &
    !$acc& create(gsgam2,gsgam2h,phi,z,lat) &
    !$acc& pcopyin(VMTR_GSGAM2,VMTR_GSGAM2H,VMTR_PHI) &
    !$acc& pcopyin(GRD_vz) &
    !$acc& pcopyin(GMTR_lat)
    ...
    !--- boundary condition
    call bndcnd_thermo_lall( ADM_gall_in, & ! [IN]
                             tem,  & ! [INOUT]
                             rho,  & ! [INOUT]
                             pre,  & ! [INOUT]
                             phi   ) ! [IN]
    ...
    do l = 1, ADM_lall
    do g = 1, ADM_gall_in
       vx(g,ADM_kmax+1,l) = vx(g,ADM_kmax,l)
       vy(g,ADM_kmax+1,l) = vy(g,ADM_kmax,l)
       vz(g,ADM_kmax+1,l) = vz(g,ADM_kmax,l)
       vx(g,ADM_kmin-1,l) = vx(g,ADM_kmin,l)
       vy(g,ADM_kmin-1,l) = vy(g,ADM_kmin,l)
       vz(g,ADM_kmin-1,l) = vz(g,ADM_kmin,l)
    enddo
    enddo
    !$acc end kernels
    ...
    do l = 1, ADM_lall
    do g = 1, ADM_gall_in
       q (g,ADM_kmax+1,l,nq) = 0.D0
       q (g,ADM_kmin-1,l,nq) = 0.D0
    enddo
    enddo
    enddo
    !$acc end kernels
    ...
    case('HELD-SUAREZ')
       write(*,*) "AF_TYPE = HELD-SUAREZ is not supported yet."
       stop
    ...
    case default
       ...
       !$acc kernels pcopy(fvx,fvy,fvz,fw,fe)
       fvx(:,:,:) = 0.D0
       fvy(:,:,:) = 0.D0
       fvz(:,:,:) = 0.D0
       fw (:,:,:) = 0.D0
       ...
       !$acc kernels pcopy(fq)
       fq (:,:,:,:) = 0.D0
       !$acc end kernels
    end select

    !$acc kernels pcopy(rhogvx,rhogvy,rhogvz,rhogw,rhoge) &
    !$acc& pcopyin(fvx,fvy,fvz,fw,fe, rho, GSGAM2,GSGAM2H)
    ...
    !$acc kernels pcopy(rhog,rhogq) pcopyin(fq,rho,GSGAM2)
    do l = 1, ADM_lall
    do k = 1, ADM_kall
    do g = 1, ADM_gall_in
       !$acc loop seq
       do nq = 1, TRC_VMAX
          frhogq = fq(g,k,l,nq) * rho(g,k,l) * GSGAM2(g,k,l)

          rhog (g,k,l)    = rhog (g,k,l)    + TIME_DTL * frhogq
          rhogq(g,k,l,nq) = rhogq(g,k,l,nq) + TIME_DTL * frhogq
       enddo
!        !$acc end loop
    enddo
    enddo
