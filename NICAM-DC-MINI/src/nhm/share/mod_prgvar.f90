...
module mod_prgvar
  ...
  real(8), public,  allocatable, save :: PRG_var   (:,:,:,:) ! container  ! for openacc
  real(8), private, allocatable, save :: PRG_var_pl(:,:,:,:)

  real(8), public,  allocatable, save :: PRG_var1   (:,:,:,:) ! container  ! for openacc
  real(8), private, allocatable, save :: PRG_var1_pl(:,:,:,:)

  real(8), public,  allocatable, save :: DIAG_var   (:,:,:,:) ! container    ! for openacc
...
contains
  ...
  subroutine prgvar_get( &
    ...
    !$acc data &
    !$acc& pcopy(rhog,rhogvx,rhogvy,rhogvz,rhogw,rhoge,rhogq) &
    !$acc& pcopyin(PRG_var,PRG_var1)

    if ( num == 0 ) then

       !$acc kernels pcopy(rhog,rhogvx,rhogvy,rhogvz,rhogw,rhoge) pcopyin(PRG_var) 
       do l = 1, ADM_lall
       do k = 1, ADM_kall
       do n = 1, ADM_gall
       ...
       !$acc kernels pcopy(rhogq) pcopyin(PRG_var) 
       do nq = 1, TRC_vmax
       do l  = 1, ADM_lall
       do k  = 1, ADM_kall
       do n  = 1, ADM_gall
    ...
    elseif( num == 1 ) then
       ...
       !$acc kernels pcopy(rhog,rhogvx,rhogvy,rhogvz,rhogw,rhoge) pcopyin(PRG_var1) 
       do l = 1, ADM_lall
       do k = 1, ADM_kall
       do n = 1, ADM_gall
       ...
       !$acc kernels pcopy(rhogq) pcopyin(PRG_var1) 
       do nq = 1, TRC_vmax
       do l  = 1, ADM_lall
       do k  = 1, ADM_kall
       do n  = 1, ADM_gall
  ...
  subroutine prgvar_get_withdiag( &
    ...
    !$acc data &
    !$acc& pcopy(rhog,rhogvx,rhogvy,rhogvz,rhogw,rhoge,rho,pre,tem,vx,vy,vz,w,rhogq,q) &
    !$acc& pcopy(DIAG_var) &
    !$acc& pcopyin(PRG_var, VMTR_GSGAM2)

    call cnvvar_prg2diag( PRG_var (:,:,:,:), PRG_var_pl (:,:,:,:), & !--- [IN]
                          DIAG_var(:,:,:,:), DIAG_var_pl(:,:,:,:)  ) !--- [OUT]

    !$acc kernels pcopy(rhog,rhogvx,rhogvy,rhogvz,rhogw,rhoge,rho,pre,tem,vx,vy,vz,w) &
    !$acc& pcopyin(PRG_var,DIAG_var, VMTR_GSGAM2) 
    do l = 1, ADM_lall
    do k = 1, ADM_kall
    do n = 1, ADM_gall
       rhog  (n,k,l) = PRG_var(n,k,l,I_RHOG  )
    ...
    !$acc kernels pcopy(rhogq,q) &
    !$acc& pcopyin(PRG_var,DIAG_var) 
    do nq = 1, TRC_vmax
    do l  = 1, ADM_lall
    do k  = 1, ADM_kall
    do n  = 1, ADM_gall
  ...
  subroutine prgvar_set( &
    ...
    !$acc& pcopyin(rhog,rhogvx,rhogvy,rhogvz,rhogw,rhoge,rhogq)

    if ( num == 0 ) then

       !$acc kernels pcopy(PRG_var) pcopyin(rhog,rhogvx,rhogvy,rhogvz,rhogw,rhoge) 
       do l = 1, ADM_lall
       do k = 1, ADM_kall
       do n = 1, ADM_gall
       ...
       !$acc kernels pcopy(PRG_var) pcopyin(rhogq) 
       do nq = 1, TRC_vmax
       do l  = 1, ADM_lall
       do k  = 1, ADM_kall
       do n  = 1, ADM_gall
       ...
       ! communication
       call COMM_data_transfer_ACC( PRG_var, PRG_var_pl )

       !$acc kernels pcopy(PRG_var) 
       do nq = 1, TRC_vmax
       do l  = 1, ADM_lall
       do k  = 1, ADM_kall
          PRG_var(suf(ADM_gall_1d,1),k,l,nq) = PRG_var(suf(ADM_gmax+1,ADM_gmin),k,l,nq)
          PRG_var(suf(1,ADM_gall_1d),k,l,nq) = PRG_var(suf(ADM_gmin,ADM_gmax+1),k,l,nq)
       enddo
       enddo
       enddo
       !$acc end kernels
    ...
    elseif( num == 1 ) then
       ...
       !$acc kernels pcopy(PRG_var1) pcopyin(rhog,rhogvx,rhogvy,rhogvz,rhogw,rhoge) 
       do l = 1, ADM_lall
       do k = 1, ADM_kall
       do n = 1, ADM_gall
       ...
       !$acc kernels pcopy(PRG_var1) pcopyin(rhogq) 
       do nq = 1, TRC_vmax
       do l  = 1, ADM_lall
       do k  = 1, ADM_kall
       do n  = 1, ADM_gall
       ...
       ! communication
       call COMM_data_transfer_ACC( PRG_var1, PRG_var1_pl )

       !$acc kernels pcopy(PRG_var1) 
       do nq = 1, TRC_vmax
       do l  = 1, ADM_lall
       do k  = 1, ADM_kall
          PRG_var1(suf(ADM_gall_1d,1),k,l,nq) = PRG_var1(suf(ADM_gmax+1,ADM_gmin),k,l,nq)
          PRG_var1(suf(1,ADM_gall_1d),k,l,nq) = PRG_var1(suf(ADM_gmin,ADM_gmax+1),k,l,nq)
       enddo
       enddo
       enddo
       !$acc end kernels
  ...
  subroutine prgvar_get_in_withdiag( &
    ...
    !$acc data pcopy(DIAG_var) pcopyin(PRG_var) &
    !$acc& pcopy(rhog,rhogvx,rhogvy,rhogvz,rhogw,rhoge,rhogq,rho,pre,tem,vx,vy,vz,w,q) &
    !$acc& pcopyin(VMTR_GSGAM2, ADM_IopJop)

    call cnvvar_prg2diag( PRG_var (:,:,:,:), PRG_var_pl (:,:,:,:), & !--- [IN]
                          DIAG_var(:,:,:,:), DIAG_var_pl(:,:,:,:)  ) !--- [OUT]

    !$acc kernels pcopy(rhog,rhogvx,rhogvy,rhogvz,rhogw,rhoge,rho,pre,tem,vx,vy,vz,w) &
    !$acc& pcopyin(PRG_var,DIAG_var, VMTR_GSGAM2, ADM_IopJop) 
    do l = 1, ADM_lall
    do k = 1, ADM_kall
    do n = 1, ADM_IopJop_nmax
       ij = ADM_IopJop(n,ADM_GIoJo)
    ...
    !$acc kernels pcopy(rhogq,q) pcopyin(PRG_var,DIAG_var, ADM_IopJop) 
    do nq = 1, TRC_vmax
    do l  = 1, ADM_lall
    do k  = 1, ADM_kall
    do n  = 1, ADM_IopJop_nmax
  ...
  subroutine prgvar_set_in( &
    ...
    use mod_comm, only: &
       ...
       COMM_data_transfer_ACC,   &    ! openacc
       COMM_data_transfer_rgn2pl_ACC  !
    use mod_runconf, only: &
       TRC_vmax
    implicit none
    ...
    !$acc data pcopy(PRG_var) &
    !$acc& pcopyin(rhog,rhogvx,rhogvy,rhogvz,rhogw,rhoge,rhogq) &
    !$acc& pcopyin(ADM_IopJop)
...
!     !$acc end loop
    ...
    ! communication
    call COMM_data_transfer_rgn2pl_ACC( PRG_var, PRG_var_pl, ADM_kall, PRG_vmax )
...
!    endif
  ...
  subroutine cnvvar_prg2diag(&
    ...
    use mod_thrmdyn, only: &
       THRMDYN_qd_ijkl,    &
       THRMDYN_tempre_ijkl,&
       THRMDYN_qd_ijkl_ACC,&    ! for openacc
       THRMDYN_tempre_ijkl_ACC
    implicit none
    ...
    !$acc data pcopy(diag) pcopyin(prg) &
    !$acc& create(qd,rho,ein) &
    !$acc& pcopyin(VMTR_GSGAM2,VMTR_GSGAM2H, GRD_afac,GRD_bfac)

    !$acc kernels pcopy(rho,diag,ein) pcopyin(prg,VMTR_GSGAM2) 
    do l  = 1, ADM_lall
    do k  = 1, ADM_kall
    do ij = 1, ADM_gall
    ...
    !$acc kernels pcopy(diag) pcopyin(prg) 
    do l  = 1, ADM_lall
    do k  = 1, ADM_kall
    do ij = 1, ADM_gall
       !$acc loop seq
...
!        !$acc end loop
    ...
    !--- calculation of dry mass concentration
    call THRMDYN_qd_ijkl_ACC ( ADM_gall, ADM_kall, ADM_lall, & !--- [IN]
                           TRC_vmax, NQW_STR, NQW_END,   & !--- [IN]
                           qd  (:,:,:),                  & !--- [OUT]
                           diag(:,:,:,I_qstr:I_qend)     ) !--- [IN]

    !--- calculation of tem, pre
    call THRMDYN_tempre_ijkl_ACC( ADM_gall, ADM_kall, ADM_lall, & !--- [IN]
                              TRC_vmax, NQW_STR, NQW_END,   & !--- [IN]
                              diag(:,:,:,I_tem),            & !--- [OUT]
                              diag(:,:,:,I_pre),            & !--- [OUT]
                              ein(:,:,:),                   & !--- [IN]
    ...
    !------ interpolation of rhog_h
    !$acc kernels pcopy(diag) pcopyin(prg,rho,VMTR_GSGAM2H,GRD_afac,GRD_bfac) 
    do l  = 1, ADM_lall
    do k  = 1, ADM_kall
    do ij = 1, ADM_gall
       if ( k == 1 ) then
          rhog_h = VMTR_GSGAM2H(ij,k+1,l) * 0.5D0 * ( GRD_afac(k+1) * rho(ij,k+1,l) &
                                                    + GRD_bfac(k+1) * rho(ij,k  ,l) )
       else
          rhog_h = VMTR_GSGAM2H(ij,k,l) * 0.5D0 * ( GRD_afac(k) * rho(ij,k  ,l) &
                                                  + GRD_bfac(k) * rho(ij,k-1,l) )
       end if
       diag(ij,k,l,I_w) = prg(ij,k,l,I_RHOGW) / rhog_h
    enddo
    enddo
    enddo
    !$acc end kernels
