...
module mod_thrmdyn
  ...
  public :: thrmdyn_ein
  public :: thrmdyn_eth
  public :: thrmdyn_eth_ldim    ! for openacc
  public :: thrmdyn_tem
  public :: thrmdyn_pre
  public :: thrmdyn_th
  public :: thrmdyn_th_ldim    ! for openacc
  ...
  public :: THRMDYN_qd_ijkl_ACC    ! for openacc
  public :: THRMDYN_rho_ijkl
  public :: THRMDYN_ein_ijkl
  public :: THRMDYN_tempre_ijkl
  public :: THRMDYN_tempre_ijkl_ACC    ! for openacc
  !-----------------------------------------------------------------------------
  !
  !++ Private parameters
  !
...
contains
  ...
  subroutine thrmdyn_eth_ldim( &
    ...
    real(8), intent(out) :: eth(1:ijdim,1:kdim,ldim)
    real(8), intent(in)  :: ein(1:ijdim,1:kdim,ldim)
    real(8), intent(in)  :: pre(1:ijdim,1:kdim,ldim)
    real(8), intent(in)  :: rho(1:ijdim,1:kdim,ldim)
    !
    integer :: ij, k, l
    !
  ...
  end subroutine thrmdyn_eth_ldim
  !-----------------------------------------------------------------------------
  ...
  subroutine thrmdyn_th( &
    ...
    do k = 1, kdim
    do ij = 1, ijdim
       th(ij,k) =tem(ij,k)*(abs(pre(ij,k))**(-CNST_KAPPA))*p0k
    enddo
    enddo
    !
  end subroutine thrmdyn_th
  !-----------------------------------------------------------------------------
  subroutine thrmdyn_th_ldim( &
       ijdim,            &  !--- IN : number of horizontal grid
       ldim,             &
       th,               &  !--- OUT : potential temperature
       tem,              &  !--- IN  : temperature       
    ...
    integer, intent(in)  :: ijdim
    integer, intent(in)  :: ldim
    real(8), intent(out) :: th (1:ijdim,1:kdim,ldim)
    real(8), intent(in)  :: tem(1:ijdim,1:kdim,ldim)
    real(8), intent(in)  :: pre(1:ijdim,1:kdim,ldim)
    !
    real(8) :: p0k
    integer :: ij, k, l
    ...
    do k = 1, kdim
    do ij = 1, ijdim
       th(ij,k,l) =tem(ij,k,l)*(abs(pre(ij,k,l))**(-CNST_KAPPA))*p0k
    enddo
    enddo
    enddo
    !$acc end kernels
  ...
  end subroutine thrmdyn_th_ldim
  !-----------------------------------------------------------------------------
...
!!$     end do
  ...
  !-----------------------------------------------------------------------------
  subroutine THRMDYN_qd_ijkl_ACC( &
       ijdim, &
       kdim,  &
       ldim,  &
       vmax,  &
    ...
    !$acc kernels pcopy(qd) pcopyin(q) 
    do l = 1, ldim
    do k = 1, kdim
    do ij = 1, ijdim
       qd(ij,k,l) = 1.D0
       !$acc loop seq
       do iw = wstr, wend
          qd(ij,k,l) = qd(ij,k,l) - q(ij,k,l,iw)
       enddo
!        !$acc end loop
    enddo
    enddo
    ...
    return
  end subroutine THRMDYN_qd_ijkl_ACC
  ...
  !-----------------------------------------------------------------------------
  subroutine THRMDYN_tempre_ijkl_ACC( &
       ijdim, &
       kdim,  &
       ldim,  &
       vmax,  &
    ...
    !$acc kernels pcopy(tem,pre) pcopyin(qd,q,CVW,ein,rho) 
    do l = 1, ldim
    do k = 1, kdim
    do ij = 1, ijdim
       cv = qd(ij,k,l) * CNST_CV
       !$acc loop seq
       do iw = wstr, wend
          cv = cv + q(ij,k,l,iw) * CVW(iw)
       enddo
!        !$acc end loop
       tem(ij,k,l) = ein(ij,k,l) / cv
       pre(ij,k,l) = rho(ij,k,l) * tem(ij,k,l) * ( qd(ij,k,l)*CNST_RAIR + q(ij,k,l,wstr)*CNST_RVAP )
    enddo
    enddo
    enddo
    !$acc end kernels
    ...
    return
  end subroutine THRMDYN_tempre_ijkl_ACC
