...
!del
  ...
  subroutine cnvvar_rhokin_ijkl( &
    ...
    real(8) :: rhogkin_h                                ! rho X ( G^{1/2} X gamma2 ) X kin (horizontal)
    real(8) :: rhogkin_v(ADM_gall,   ADM_kall,ADM_lall) ! rho X ( G^{1/2} X gamma2 ) X kin (vertical)
    real(8) :: rhog_h
    ...
    !$acc& create(rhogkin_v) &
    !$acc& pcopyin(GRD_afac,GRD_bfac,GRD_cfac,GRD_dfac) &
    !$acc& pcopyin(VMTR_RGSGAM2,VMTR_GSGAM2H)

    !--- rhog at the half level
    !--- vertical kinetic energy
    !$acc kernels pcopy(rhogkin_v) pcopyin(rhog,rhogw, GRD_afac,GRD_bfac,VMTR_RGSGAM2,VMTR_GSGAM2H) 
    !$acc loop gang
    do l = 1, ADM_lall
    ...
    do ij = 1, ADM_gall
       if (( k >= ADM_kmin+1 ) .and. ( k <= ADM_kmax )) then
          rhog_h = 0.5D0 * ( GRD_afac(k) * VMTR_RGSGAM2(ij,k  ,l) * rhog(ij,k  ,l) &
                           + GRD_bfac(k) * VMTR_RGSGAM2(ij,k-1,l) * rhog(ij,k-1,l) &
                           ) * VMTR_GSGAM2H(ij,k,l)
          rhogkin_v(ij,k,l) = 0.5D0 * ( rhogw(ij,k,l) * rhogw(ij,k,l) ) / rhog_h
       else 
          rhogkin_v(ij,k,l) = 0.D0
       endif
    enddo
    enddo
    enddo
    ...
    !$acc kernels pcopy(rhogkin) pcopyin(rhogvx,rhogvy,rhogvz,rhog,rhogkin_v, GRD_cfac,GRD_dfac) 
    !$acc loop gang
    do l = 1, ADM_lall
    !$acc loop gang vector(4)
    ...
    do ij = 1, ADM_gall
       if (( k >= ADM_kmin ) .and. ( k <= ADM_kmax )) then
          rhogkin_h = 0.5D0 * ( rhogvx(ij,k,l) * rhogvx(ij,k,l) &
                              + rhogvy(ij,k,l) * rhogvy(ij,k,l) &
                              + rhogvz(ij,k,l) * rhogvz(ij,k,l) ) / rhog(ij,k,l)
          rhogkin(ij,k,l) = rhogkin_h                           &
               + 0.5D0 * ( GRD_dfac(k) * rhogkin_v(ij,k+1,l) &
                         + GRD_cfac(k) * rhogkin_v(ij,k  ,l) )
       else
          rhogkin(ij,k,l) = 0.D0
       endif
    enddo
