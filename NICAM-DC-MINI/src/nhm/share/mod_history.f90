...
module mod_history
  ...
  integer,                     public,  allocatable, save :: ksumstr  (:)    ! openacc
  ...
  real(8),                     public,  allocatable, save :: v_save   (:,:,:,:)    ! openacc
  ...
  integer,                     public,  allocatable, save :: cnvpre_klev(:,:,:)    ! openacc
  real(8),                     public,  allocatable, save :: cnvpre_fac1(:,:,:)    !
  real(8),                     public,  allocatable, save :: cnvpre_fac2(:,:,:)    !
  !-----------------------------------------------------------------------------
contains
  !-----------------------------------------------------------------------------
...
!          enddo
  ...
  subroutine  history_in( item, gd, l_region )
    ...
    if ( calc_pressure ) then
       write(*,*) "calc_pressure is not supported yet."
       stop
          ...
          if ( .NOT. out_prelev_save(n) ) then ! normal
             ...
             if ( ktype_save(n) == '3D' ) then ! trim HALO
                ...
                if (ijdim_input == ADM_gall_in) then
                   ...
                   !$acc kernels pcopy(v_save) pcopyin(gd, ksumstr,ADM_IopJop)
...
!                    !$acc end loop
             ...
             if (ijdim_input == ADM_gall_in) then
                ...
                !$acc kernels pcopy(v_save) pcopyin(gd, cnvpre_fac1,cnvpre_fac2,cnvpre_klev,ADM_IopJop)
                do k2 = 1, npreslev
                !$acc loop independent
                do g  = 1, ADM_IopJop_nmax
                   ! k  = cnvpre_klev(g2,k,l)   ! may be wrong (by anaruse)
                   g2 = ADM_IopJop(g,ADM_GIoJo)
...
!                 !$acc end loop
                ...
                !$acc kernels pcopy(v_save) pcopyin(gd, cnvpre_fac1,cnvpre_fac2,ksumstr)
                !$acc loop independent
                do k = 1, kmax
                do g = 1, ADM_gall
                   k2 = ksumstr(n)-1 + k
...
!                 !$acc end loop
  ...
  subroutine history_out
    ...
    !--- !$acc data pcopy(v_save)
    !--- !$acc update host(v_save)
    ...
    do n = 1, HIST_req_nmax
       ...
       if ( out_var(n) ) then
          ...
          do k = ksumstr(n), ksumend(n)
          do g = 1, ADM_gall_pl
             v_save_pl(g,k,l,1) = v_save_pl(g,k,l,1) / tsum_save(n,1)  ! could be wrong (by anaruse)
          enddo
          enddo
          enddo
