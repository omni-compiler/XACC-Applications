...
!!$  integer, allocatable, public,  save :: index_sfc_pl(:,:,:)
  ...
  !++ Private variables
  !
  real(8), allocatable, public,   save :: sfcvar(:,:,:,:)    ! openacc
  real(8), allocatable, private,  save :: sfcvar_pl(:,:,:,:)
  integer, allocatable, private,  save :: KMAX(:)
  integer, allocatable, public,   save :: KSTR(:)    ! openacc
...
!    sfcvar_pl(:,:,:,:)=VMISS
  ...
  subroutine sfcvar_get( &
    ...
    integer ::  g, l
    !
    !$acc kernels pcopy(sv) pcopyin(sfcvar,KSTR)
    do l = 1, ADM_lall
    do g = 1, ADM_gall
       sv(g,ADM_KNONE,l) = sfcvar(g,KSTR(vid),l,1)
    enddo
    enddo
    !$acc end kernels
  ...
  subroutine sfcvar_get2(  &
    ...
    !$acc data pcopy(sv) pcopyin(sfcvar,KSTR)
