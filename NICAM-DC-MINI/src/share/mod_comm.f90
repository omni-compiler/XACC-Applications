...
!coarray  use mpi
  ...
  public :: COMM_data_transfer_ACC        ! openacc
  public :: COMM_data_transfer_rgn2pl_ACC ! openacc
...
!  integer,private,save::maxdatasize
  ...
  !  for send
  !--------------------------------------------------
  integer,allocatable,public,save::nsmax(:,:)           ! openacc, private -> public
  integer,allocatable,public,save::sendinfo(:,:,:,:)
  integer,allocatable,public,save::sendlist(:,:,:,:)
  integer,allocatable,public,save::nsmax_pl(:,:)        ! openacc, private -> public
  integer,allocatable,public,save::sendinfo_pl(:,:,:,:) ! openacc, private -> public
  integer,allocatable,public,save::sendlist_pl(:,:,:,:) ! openacc, private -> public
  !
  !--------------------------------------------------
  !  for copy
  !--------------------------------------------------
  integer,allocatable,public,save::ncmax_r2r(:)        ! openacc, private -> public
  integer,allocatable,public,save::copyinfo_r2r(:,:,:) ! openacc, private -> public
  integer,allocatable,public,save::recvlist_r2r(:,:,:) ! openacc, private -> public
  integer,allocatable,public,save::sendlist_r2r(:,:,:) ! openacc, private -> public
  !--------------------------------------------------
  integer,allocatable,public,save::ncmax_r2p(:)        ! openacc, private -> public
  integer,allocatable,public,save::copyinfo_r2p(:,:,:) ! openacc, private -> public
  integer,allocatable,public,save::recvlist_r2p(:,:,:) ! openacc, private -> public
  integer,allocatable,public,save::sendlist_r2p(:,:,:) ! openacc, private -> public
  !--------------------------------------------------
  integer,allocatable,public,save::ncmax_p2r(:)        ! openacc, private -> public
  integer,allocatable,public,save::copyinfo_p2r(:,:,:) ! openacc, private -> public
  integer,allocatable,public,save::recvlist_p2r(:,:,:) ! openacc, private -> public
  integer,allocatable,public,save::sendlist_p2r(:,:,:) ! openacc, private -> public
  !--------------------------------------------------
  integer,allocatable,public,save::ncmax_sgp(:)        ! openacc, private -> public
  integer,allocatable,public,save::copyinfo_sgp(:,:,:) ! openacc, private -> public
  integer,allocatable,public,save::recvlist_sgp(:,:,:) ! openacc, private -> public
  integer,allocatable,public,save::sendlist_sgp(:,:,:) ! openacc, private -> public
  !--------------------------------------------------
  !
  !--------------------------------------------------
  !  for recv
  !--------------------------------------------------
  integer,allocatable,public,save::nrmax(:,:)           ! openacc, private -> public
  ...
  integer,allocatable,public,save::nrmax_pl(:,:)        ! openacc, private -> public
  integer,allocatable,public,save::recvinfo_pl(:,:,:,:) ! openacc, private -> public
  integer,allocatable,public,save::recvlist_pl(:,:,:,:) ! openacc, private -> public
  !--------------------------------------------------
  ! 
...
!    do ro=1,romax(halo)
!      do n=1,rsize(ro,halo)*cmax
!        recvbuf(n,ro) = recvbufc(n,ro)
!        recvbuf(n,ro) = recvbufA(n,ro)
!      end do
!    end do
...
!          v_spl_recvcc(:,:)[ADM_prc_spl] = v_spl_send(:,:)
          endif
          sync all
...
! end module mod_comm
  !-----------------------------------------------------------------------------
  subroutine COMM_data_transfer_ACC(&
       var,var_pl,              & !--- INOUT : variables comminicated.
       trn,                     & !--- IN : commnucation flag for each variable
       hallo_num)                 !--- IN : number of hallo
    use mod_adm, only :      &
...
!    integer :: i_dbg !iga
    ...
    integer :: max_ssize    ! for openacc
    integer :: max_rsize    ! for openacc
...
!        call MPI_BARRIER( ADM_comm_run_world, ierr )
    ...
    call DEBUG_rapstart('COMM data_transfer ACC')
...
!cdir novector
    ...
    max_rs_pl = 0
    do ro=1,romax(halo)
       if ( max_rsize < rsize(ro,halo) ) then
          max_rsize = rsize(ro,halo)
       endif

       if ( max_nrmax < nrmax(ro,halo) ) then
          max_nrmax = nrmax(ro,halo);
       endif
       do nr=1,nrmax(ro,halo)
          rs=recvinfo(SIZE_COMM,nr,ro,halo)
          if ( max_rs < rs ) then
       ...
       if ( max_nrmax_pl < nrmax_pl(ro,halo) ) then
          max_nrmax_pl = nrmax_pl(ro,halo);
       endif
       do nr=1,nrmax_pl(ro,halo)
          rs=recvinfo_pl(SIZE_COMM,nr,ro,halo)
          if ( max_rs_pl < rs ) then
             max_rs_pl = rs
          endif
       enddo
    enddo
    ...
    max_ss_pl = 0
    do so=1,somax(halo)
       if ( max_ssize < ssize(so,halo) ) then
          max_ssize = ssize(so,halo)
       endif

       if ( max_nsmax < nsmax(so,halo) ) then
          max_nsmax = nsmax(so,halo);
       endif
       do ns=1,nsmax(so,halo)
          ss=sendinfo(SIZE_COMM,ns,so,halo)
          if ( max_ss < ss ) then
       ...
       if ( max_nsmax_pl < nsmax_pl(so,halo) ) then
          max_nsmax_pl = nsmax_pl(so,halo);
       endif
       do ns=1,nsmax_pl(so,halo)
          ss=sendinfo_pl(SIZE_COMM,ns,so,halo)
          if ( max_ss_pl < ss ) then
             max_ss_pl = ss
          endif
       enddo
    enddo
    ...
    do nc=1,ncmax_sgp(halo)
       cs=copyinfo_sgp(SIZE_COPY,nc,halo)
       if ( max_cs_sgp < cs ) then
          max_cs_sgp = cs
       endif
    enddo

    cur_somax = somax(halo)
    cur_romax = romax(halo)
    cur_ncmax_r2r = ncmax_r2r(halo)
    cur_ncmax_r2p = ncmax_r2p(halo)
    cur_ncmax_p2r = ncmax_p2r(halo)
    cur_ncmax_sgp = ncmax_sgp(halo)
    ...
    if ( first > 0 ) then
       ...
       write(*,*) "kmax = ", kmax
       write(*,*) "cmax = ", cmax
       write(*,*) "ssize(so,halo) = ", (ssize(so,halo), so=1,somax(halo))
       write(*,*) "rsize(ro,halo) = ", (rsize(ro,halo), ro=1,romax(halo))
       write(*,*) "max_rsize = ", max_rsize
       write(*,*) "max_ssize = ", max_ssize
       write(*,*) "max_nsmax = ", max_nsmax
       write(*,*) "max_nsmax_pl = ", max_nsmax_pl
       write(*,*) "max_ss = ", max_ss
       write(*,*) "max_ss_pl = ", max_ss_pl
       write(*,*) "max_nrmax = ", max_nrmax
       write(*,*) "max_nrmax_pl = ", max_nrmax_pl
       write(*,*) "max_rs = ", max_rs
       write(*,*) "max_rs_pl = ", max_rs_pl
       write(*,*) "somax = ", somax(halo)
       write(*,*) "romax = ", somax(halo)
       write(*,*) "ncmax_r2r = ", ncmax_r2r(halo)
       write(*,*) "ncmax_r2p = ", ncmax_r2p(halo)
       write(*,*) "ncmax_p2r = ", ncmax_p2r(halo)
       write(*,*) "ncmax_sgp = ", ncmax_sgp(halo)
       write(*,*) "max_cs_r2r = ", max_cs_r2r
       write(*,*) "max_cs_r2p = ", max_cs_r2p
       write(*,*) "max_cs_p2r = ", max_cs_p2r
       write(*,*) "max_cs_sgp = ", max_cs_sgp
    endif

!     !$acc data present(var) pcopy(var_pl) pcopyin(clist) 
    !$acc data present(var) pcopy(var_pl) pcopyin(clist)
...
!coarray    t(4) = mpi_wtime()
    t(4) = xmp_wtime()
    do so=1,cur_somax
       cur_ssize = ssize(so,halo)*cmax
       cur_nsmax = nsmax(so,halo)
       cur_nsmax_pl = nsmax_pl(so,halo)
...
!coarray    t(7) = mpi_wtime()
    ...
    !$acc& pcopyin(recvlist_r2r,sendlist_r2r,copyinfo_r2r) 
    !$acc loop independent gang
    do m=1,varmax
       !$acc loop independent gang
    ...
    !$acc& pcopyin(recvlist_r2p,sendlist_r2p,copyinfo_r2p) 
    !$acc loop independent gang
    do m=1,varmax
       !$acc loop independent gang
    ...
    !-----------------------------------------
    !$acc kernels present(var,var_pl) &
    !$acc& pcopyin(recvlist_p2r,sendlist_p2r,copyinfo_p2r) 
    !$acc loop independent gang
    do m=1,varmax
       !$acc loop independent gang
...
!     !$acc& pcopyin(nrmax,nrmax_pl) 
    ...
    do ro=1,cur_romax
       ...
       cur_rsize = rsize(ro,halo)*cmax
       cur_nrmax = nrmax(ro,halo)
       cur_nrmax_pl = nrmax_pl(ro,halo)
...
!    write(ADM_log_fid,*) 'recv count=',i_dbg,'prc=',adm_prc_me
    ...
    !-----------------------------------------
    !$acc kernels present(var) &
    !$acc& pcopyin(recvlist_sgp,sendlist_sgp,copyinfo_sgp) 
    !$acc loop independent gang
    do m=1,varmax
       !$acc loop independent gang
...
!coarray    t(11) = mpi_wtime()
    ...
    !time_bar2=time_bar2+(t(12)-t(11))
    !time_total=time_total+(t(12)-t(0))
    time_total=time_total+(t(11)-t(0))

    !$acc end data

    !$acc wait
    call DEBUG_rapend('COMM data_transfer ACC')

    ! debug
    first = first - 1

  end subroutine COMM_data_transfer_ACC

  !-----------------------------------------------------------------------------
  subroutine COMM_data_transfer_rgn2pl_ACC( &
       var,    &
       var_pl, &
       knum,   &
       nnum    )
...
!        endif
       ...
       do l = 1, ADM_lall
          ...
          sync all !20180126
          if ( rgnid == ADM_rgnid_npl_mng ) then
             !$acc kernels copyout(v_npl_send) pcopyin(var) 
             do n = 1, nnum
             do k = 1, knum
                v_npl_send(k,n) = var(suf(ADM_gmin,ADM_gmax+1),k,l,n)
             enddo
...
!                              ierr                  )
          ...
          !--- south pole
          if ( rgnid == ADM_rgnid_spl_mng ) then
             !$acc kernels copyout(v_spl_send) pcopyin(var) 
             do n = 1, nnum
             do k = 1, knum
                v_spl_send(k,n) = var(suf(ADM_gmax+1,ADM_gmin),k,l,n)
             enddo
...
!              var_pl(ADM_GSLF_PL,k,ADM_SPL,n) = v_spl_recv(k,n)
    ...
    return
  end subroutine COMM_data_transfer_rgn2pl_ACC
